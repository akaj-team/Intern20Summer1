...
class GraphView(context: Context?, attrs: AttributeSet?) : View(context, attrs) {
    ...
    companion object {
        ...
        private const val STEP_VERTICAL = 10
    }
    ...
    private fun drawConnectLineWeight(canvas: Canvas) {
        ...
        data.forEachIndexed { index, weight ->
            ...
            // draw vertical dash from point to oY
            canvas.drawLine(x1, y1, x1, zeroPoint.y, paintDashed())
            // draw horizontal dash from point to oX
            canvas.drawCircle(x1, zeroPoint.y, 20f, paintPoint2())
            canvas.drawLine(x1, y1, x0, y1, paintDashed())

            canvas.drawCircle(x1, y1, 10f, paintPoint())
            canvas.drawText(weight.weight.toString(), x1 - 10f, y1 - 10f, paintText())
            canvas.drawText("day " + weight.day.toString(), x1, y0 - 20, paintText())
    ...
    private fun paintPoint2() = Paint().apply {
        Paint.ANTI_ALIAS_FLAG
        color = Color.BLUE
        style = Paint.Style.FILL
        pathEffect = null
