...
class HomeViewModel(private val repository: RemoteRepository) : ViewModel(), HomeDataSource {
    override fun getListPost(token: String): Single<Response<MutableList<Post>>>? = repository.getListPost(token)

    override fun updateLikePost(token: String, id: Int): Single<Response<LikeResponse>>? = repository.updateLikePost(token, id)
}
